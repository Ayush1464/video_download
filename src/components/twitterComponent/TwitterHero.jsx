...
const TwitterHero = () => {
  ...
  const handleSearch = async () => {
    if (!videoURL.trim()) {
      toast.error("Please enter a Twitter video URL");
      return;
    }

    const xRegex = /^(https?:\/\/)?(www\.)?(twitter\.com|x\.com)\/.+$/;
    if(!xRegex.test(videoURL)) {
      toast.error("Please enter a valid Twitter video URL");
      return;
    }
    ...
    try {
      ...
      toast.error("Video not found on this tweet");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full bg-gray-100 px-4 text-center">
      ...
      <p className="text-lg mb-8 text-gray-600">
        Paste the Twitter video link below to download in seconds.
      </p>
